...
(function () {
  ...
  // Đọc JSON sản phẩm từ script nhúng trong HTML
  ...
  // Đóng modal khi click ra nền hoặc nhấn Esc
  ...
  function mockFallback(products){
    ...
    return arr.slice(0,2).map(p => ({ id: p.id, qty: 1, size: "M" }));
  }
  ...
  function setAddrMode(mode){
    ...
    if (accForm) accForm.style.display = isNew ? "none" : "block";
